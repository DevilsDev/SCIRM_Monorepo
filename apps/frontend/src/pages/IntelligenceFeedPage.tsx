    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Real-time supply chain intelligence from news, weather, regulatory, and financial sources</p>
        </div>
        <button
        ...
        {['', 'news', 'weather', 'regulatory', 'financial'].map((src) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors capitalize ${
              sourceFilter === src ? 'bg-blue-100 text-blue-800 border-blue-300' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'
            }`}
          >
            {src || 'All Sources'}
          </button>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {docs.map((doc) => (
            <div key={doc.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${SOURCE_COLORS[doc.source] || 'bg-gray-100 text-gray-800'}`}>
                      {doc.source}
                    </span>
                    <SeverityBadge severity={doc.severity} />
                    <span className="text-xs text-gray-400">{doc.region}</span>
                  </div>
                  <h3 className="text-sm font-semibold text-gray-900 mt-1">{doc.title}</h3>
