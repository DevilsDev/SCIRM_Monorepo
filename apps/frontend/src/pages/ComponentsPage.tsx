...
export default function ComponentsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Components & BOM</h1>
        <p className="text-sm text-gray-500 mt-1">{components.length} components across {products.length} products</p>
      </div>
          ...
          {products.map((p) => (
            <div
              ...
              className={`bg-white rounded-xl shadow-sm border p-5 cursor-pointer transition-all hover:shadow-md ${
                selectedProduct?.id === p.id ? 'border-blue-500 ring-2 ring-blue-100' : 'border-gray-200'
              }`}
            >
              <h3 className="text-sm font-semibold text-gray-900">{p.name}</h3>
              <div className="grid grid-cols-3 gap-2 mt-3">
                <div className="text-center">
                  <p className="text-lg font-bold text-gray-900">{p.component_count}</p>
                ...
                <div className="text-center">
                  <p className="text-lg font-bold text-gray-900">{p.max_component_risk}</p>
                  <p className="text-[10px] text-gray-500">Max Risk</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {selectedProduct && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 bg-gray-50 border-b">
            <h2 className="text-lg font-semibold text-gray-900">BOM: {selectedProduct.name}</h2>
          </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {bom.map((item: any) => (
                <tr key={item.id} className={item.is_critical ? 'bg-red-50/30' : ''}>
                  ...
                  <td className="px-6 py-3 text-sm text-gray-500 font-mono">{item.part_number}</td>
                  <td className="px-6 py-3 text-sm text-gray-500 capitalize">{item.category?.replace('_', ' ')}</td>
                  <td className="px-6 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${critColors[item.criticality] || 'bg-gray-100'}`}>
                      {item.criticality}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-3 text-xs text-gray-500">{(item.suppliers || []).join(', ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Component Catalog */}
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-3">Component Catalog</h2>
          ...
          {components.map((c: any) => (
            <div key={c.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${critColors[c.criticality] || 'bg-gray-100'}`}>
                  {c.criticality}
                </span>
              </div>
              <p className="text-sm font-medium text-gray-900">{c.name}</p>
              <p className="text-xs text-gray-500 capitalize mt-1">{c.category?.replace('_', ' ')}</p>
