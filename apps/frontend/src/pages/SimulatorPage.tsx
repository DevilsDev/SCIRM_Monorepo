...
export default function SimulatorPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Digital Twin Simulator</h1>
        <p className="text-sm text-gray-500 mt-1">Monte Carlo simulation — model disruption scenarios across your supply chain</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <label className="block text-sm font-medium text-gray-700 mb-2">Disrupt Suppliers</label>
          <div className="space-y-2 mb-4">
            {SUPPLIERS.map((s) => (
              <label key={s} className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={selected.includes(s)} onChange={() => toggleSupplier(s)} className="rounded border-gray-300" />
                {s}
              </label>
            ))}
          </div>
          ...
          <select value={severity} onChange={(e) => setSeverity(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-4">
          ...
          <input type="number" value={duration} onChange={(e) => setDuration(Number(e.target.value))} min={1} max={365} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-4" />

          <label className="block text-sm font-medium text-gray-700 mb-1">Simulations</label>
          <input type="number" value={simCount} onChange={(e) => setSimCount(Number(e.target.value))} min={100} max={10000} step={100} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-4" />
            ...
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
                  <p className="text-2xl font-bold text-gray-900">${(result.financial_impact_p50 / 1000).toFixed(0)}k</p>
                  <p className="text-xs text-gray-500">Median Impact (P50)</p>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
                  <p className="text-2xl font-bold text-gray-900">{result.recovery_days_p50.toFixed(0)}d</p>
                  <p className="text-xs text-gray-500">Median Recovery</p>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
                  <p className="text-2xl font-bold text-gray-900">{result.suppliers_affected_mean.toFixed(1)}</p>
                  <p className="text-xs text-gray-500">Avg Suppliers Hit</p>
                </div>
              </div>

              {/* Financial Impact Distribution */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Financial Impact Distribution ({result.num_simulations.toLocaleString()} simulations)</h3>
                <AnimatedBars
